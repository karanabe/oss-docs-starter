---
title: コンテンツ表示サンプル
description: よく使うドキュメント表現、数式、Mermaid図、コンポーネント、コードハイライトを確認できます。
...
インライン数式は、$E = mc^2$ のように1つのドル記号で囲みます。長い数式は2つのドル記号で囲み、独立した行に表示します。

$$
\sum_{k=1}^{n} k = \frac{n(n+1)}{2}
$$

数式は本番ビルド時にKaTeXで描画されます。クライアント側のJavaScriptを使わず、支援技術でも内容を読み取れます。
...
長いガイドは関連する作業ごとに分けると、読み始める場所を探しやすくなり、後から手順を見返すときにも役立ちます。補足する情報は、対応する作業の下にまとめます。

#### 前提条件を確認する

対応するOSと必要なツールを、インストール手順の前に示します。事前の環境準備が必要な場合は、設定手順へのリンクを添えます。
