...
Notesは、日付付きの近況、判断、実験を残すための任意機能です。通常のMarkdownページを `src/content/docs/notes/` に置き、対応する日本語ファイルを `src/content/docs/ja/notes/` に置きます。各言語のランディングページで概要を説明し、公開済みメモを日付の新しい順に表示します。Notesはヘッダーから開き、ドキュメントの左サイドバーには表示しません。
...
メモを追加するには、サンプルの `notes/welcome.md` を安定した分かりやすいファイル名へコピーし、frontmatterと本文を更新します。日本語ページも同じ相対パスに作成してください。Notes一覧では `publishedAt` を基準に並べ、公開済みメモの意味を変更した場合は `updatedAt` を追加します。
